...
export class OrderRepository {
    ...
    async create(userId, shippingAddress, billingAddress) {
    ...
    async updateStatus(orderId, status) {
        ...
        if (!validTransitions[order.status]?.includes(status)) {
            throw new BusinessError(`Cannot transition from ${order.status} to ${status}`, ErrorCode.ORDER_NOT_CANCELLABLE);
        }
        try {
